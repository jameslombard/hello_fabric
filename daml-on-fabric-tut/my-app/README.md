...
Deploying `my-app` to the hosted Daml platform
...
DAR `my-app.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/my-app-0.1.0.dar -o ui/daml.js
(cd ui && npm run-script build && zip -r ../my-app-ui.zip build)
```
...
(`my-app-ui.zip`) you have just created and publish it.

To see your deployed instance of `my-app` in action, follow the
...
- `daml.js/my-app-0.1.0/src/User.ts` (once you've generated it):
