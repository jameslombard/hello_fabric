    ...
    "uploadPackages" should {
      ...
      "remove a duplicate package from the update" in participantState.use { ps =>
        ...
        for {
          ...
          all(Seq(update1, update2, update3).map(_.recordTime)) should be >= rt
    ...
    "process many party allocations" in participantState.use { ps =>
      ...
      for {
        results <- Future.sequence(
          partyNames.map(
            name => ps.allocateParty(Some(name), Some(name), newSubmissionId()).toScala
          )
        )
        _ = all(results) should be(SubmissionResult.Acknowledged)
        updates <- updatesF
...
object ParticipantStateIntegrationSpecBase {
  ...
  private val archives = darReader
    .readArchive(
      "Test-1.5.dar",
      new ZipInputStream(this.getClass.getClassLoader.getResourceAsStream("Test-1.5.dar"))
    )
    .get
  ...
  private def transactionMeta(aLedgerEffectiveTime: Timestamp) =
    TransactionMeta(
